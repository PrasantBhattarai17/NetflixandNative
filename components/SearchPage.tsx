...
import { APIoptions, search, Search_API, searchTail, Top_API, voice } from './utils/constants'
import ListSearch from './ListSearch'
import { useDispatch} from "react-redux";
import { addConstantdata, addSearchData } from './store/SearchSlice'
import usePopularMovies from './hooks/usePopularMovies'
const SearchPage = () => {
    const [searchData,setSearchData]=useState('');
    const dispatch=useDispatch();
    const topdata=usePopularMovies(Top_API);
    dispatch(addConstantdata(topdata));
...
<View className='flex-1 '>
    <View className='flex -1 w-full h-16  bg-[#424242] flex-row items-center  justify-evenly px-2 '>
